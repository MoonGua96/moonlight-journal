      ...
      {categoryDialog && (
        <CategoryEditor
          ...
          onSave={(name, color) => {
            ensureCategories();
            setState((current) => {
              const source = current.ledgerCategories.length
                ? current.ledgerCategories
                : categories;
              return {
                ...current,
                ledgerCategories: categoryDialog.category
                  ? source.map((item) =>
                      item.id === categoryDialog.category?.id
                        ? { ...item, name, color }
                        : item,
                    )
                  : [
...
function CategoryEditor({
  ...
  const [color, setColor] = useState(
    value?.color || (type === "income" ? "#5e8f78" : "#9a647d"),
  );
  const swatches = palette.map((item) => item.base);
  return (
    <div className="modal-layer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal category-editor" role="dialog" aria-modal="true" aria-label={value ? "編輯分類" : "新增分類"}>
        <header>
          <div><small>LEDGER CATEGORY</small><h2>{value ? "編輯分類" : `新增${type === "income" ? "收入" : "支出"}分類`}</h2></div>
          <button className="close" aria-label="關閉" onClick={onClose}>×</button>
        </header>
        <div className="modal-body">
          <label className="field">
            <span>分類名稱</span>
            <input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：生活雜支" />
          </label>
          <div className="category-color-editor">
            <span>分類顏色</span>
